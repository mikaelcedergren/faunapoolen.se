...
export function classifyChanges(
  ...
  const privateInterfacePattern =
    /^src\/app\/pages\/(?:admin\/|campaigns\/)|^src\/app\/(?:app\.component|app\.routes\.ts$)/u;
  const e2ePattern = /^(?:e2e\/|playwright\.config\.ts$|scripts\/(?:run-e2e|e2e-server)\.mjs$)/u;
  const highRiskPattern =
    /^(?:AGENTS\.md|CLAUDE\.md|\.agents\/|\.codex\/|\.gitignore$|package\.json|pnpm-lock\.yaml|pnpm-workspace\.yaml|angular\.json|tsconfig(?:\.[^.]+)?\.json|cx-product\.json|proxy\.conf\.json|\.github\/|\.env\.(?:web|worker)\.example$|server\/|launchd\/|bin\/|ops\/|scripts\/(?:verify-change|dev|flatten)\.mjs$|tests\/(?:server\/|change-verification\.test\.mjs$)|src\/(?:main\.server\.ts$|app\/app\.config\.server\.ts$|app\/app\.routes\.ts$)|public\/robots\.txt$|DEVELOPMENT-VERIFICATION\.md$)/u;
  const interfaceFiles = changedFiles.filter((file) => interfacePattern.test(file));
  const privateInterfaceChange = has(privateInterfacePattern);
  ...
  if (highRisk) {
    checks.add('full');
    if (e2eChange || privateInterfaceChange) checks.add('e2e');
    ...
    if (interfaceFiles.length > 0) {
      ...
      if (privateInterfaceChange) checks.add('e2e');
      checks.add('visual');
      routes.push(...interfaceFiles.map(routeForSourcePath));
    }
    if (e2eChange) {
...
function plannedChecks(classification) {
  const checks = {
    full: commandCheck('full', 'Full Faunapoolen proof', 0, ['corepack', 'pnpm', 'check'], {
      hostAccess: true,
    }),
    ...
    visual: {
      hostAccess: true,
      id: 'visual',
      label: `Rendered Faunapoolen check (${classification.routes.join(', ')})`,
      phase: 2,
...
export function checkInputHash(check, snapshot) {
  const hash = createHash('sha256');
  hash.update(
    `faunapoolen-change-verification-v1\0${process.version}\0${check.id}\0${JSON.stringify(check.command ?? check.routes ?? [])}`,
  );
...
function printPlan(classification, changedFiles, checks, reusable) {
  console.log('Faunapoolen change verification');
