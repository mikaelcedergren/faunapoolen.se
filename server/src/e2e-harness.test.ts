...
} from '../../e2e/web-server-environment.mjs';
...
test('the product E2E layer is thin and delegates containment to cx-framework', () => {
  ...
  assert.equal(server.CX_TEST_ALLOWED_ORIGIN, 'http://127.0.0.1:50124');
  ...
  const fetchGuard = readFileSync(
    path.join(repoRoot, 'server', 'test', 'block-external-fetch.mjs'),
    'utf8',
  );
  const smoke = readFileSync(path.join(repoRoot, 'e2e', 'smoke.spec.ts'), 'utf8');
