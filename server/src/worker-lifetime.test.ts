...
test('a disabled worker remains alive without a provider until graceful shutdown', async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'faunapoolen-idle-worker-'));
  ...
  const child = spawn(
    ...
    const { startFaunapoolenWorker } = await import(${JSON.stringify(moduleUrl)});
    const runtime = await startFaunapoolenWorker({ entrypointUrl: ${JSON.stringify(moduleUrl)} });
