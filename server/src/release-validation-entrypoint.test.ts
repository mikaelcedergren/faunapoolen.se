...
test('compiled release-validation web entrypoint is secretless, ready, and denies the development credentials', async () => {
  ...
  try {
    ...
    assert.deepEqual(await health.json(), { app: 'faunapoolen', ok: true, port });

    const denied = await fetch(`${server.baseUrl}/api/admin/login`, {
      body: JSON.stringify({
...
function temporaryRoot(): string {
  ...
  const root = fs.realpathSync(
    fs.mkdtempSync(path.join(canonicalTemp, 'faunapoolen-release-validation-entrypoint-')),
  );
...
function writeIdentity(root: string): string {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      nodeMajor: 26,
      releaseId: 'faunapoolen-secretless-validation-test',
      revision: 'b'.repeat(40),
      schemaVersion: 1,
      serverBuildId: `server-${artifactDigest}`,
      sourceDirty: true,
      sourceFingerprint: 'c'.repeat(64),
      workers: [{ entrypoint: 'server/dist/worker.js', key: 'jobs' }],
    })}\n`,
    { flag: 'wx', mode: 0o600 },
  );
  return identityFile;
...
async function waitForHealth(server: RunningServer): Promise<Response> {
  ...
  while (Date.now() < deadline) {
    if (server.child.exitCode !== null || server.child.signalCode !== null) {
      throw new Error(
        `Faunapoolen exited before release-validation readiness.\n${server.output()}`,
      );
    }
    try {
  ...
  throw new Error(`Faunapoolen release validation did not become ready.\n${server.output()}`);
}
...
async function waitForExit(
  ...
  return await new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      server.child.kill('SIGKILL');
      reject(new Error(`Faunapoolen release-validation child did not exit.\n${server.output()}`));
    }, 10_000);
...
async function availablePort(): Promise<number> {
  ...
  if (!address || typeof address === 'string') {
    probe.close();
    throw new Error('Could not reserve a loopback port for Faunapoolen release validation.');
  }
  await new Promise<void>((resolve, reject) => {
...
function removeTemporaryRoot(root: string): void {
  ...
  assert.match(path.basename(root), /^faunapoolen-release-validation-entrypoint-[A-Za-z0-9]+$/);
