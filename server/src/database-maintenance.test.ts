...
import { FAUNAPOOLEN_MIGRATIONS } from './database.js';
...
test('compiled offline migration holds legacy work and immutable verification never changes the store', (t) => {
  const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'faunapoolen-maintenance-')));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const data = path.join(root, 'data');
  fs.mkdirSync(data, { mode: 0o700 });
  const filename = path.join(data, 'faunapoolen.db');
  ...
  assert.deepEqual(fs.readdirSync(data), ['faunapoolen.db']);
