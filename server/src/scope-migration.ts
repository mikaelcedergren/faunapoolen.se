...
export function assignFaunapoolenLegacyJobScopes(
  ...
  return assignLegacyDurableJobScopes({
    ...
    afterAssign() {
      for (const { id, executionScope } of assignments) {
        database.run(
          "UPDATE generation_runs SET execution_scope = ? WHERE job_id = ? AND execution_scope = 'legacy'",
          [executionScope, id],
        );
        if (
          database.get('SELECT 1 FROM generation_runs WHERE job_id = ? AND execution_scope <> ?', [
            id,
            executionScope,
          ])
        ) {
