    ...
    for (let ordinal = 1; ordinal <= 2; ordinal += 1) {
      ...
      if (ordinal === 2) {
        throw new GenerationProviderTerminalError(
          ...
          'The provider returned invalid structured campaign content twice.',
          'failed',
        );
      }
    }
        ...
        if (!response.ok) {
          ...
          if (DEFINITIVE_CREATE_REJECTION_STATUSES.has(response.status)) {
            effect = repository.transitionEffect({
              ...
              errorMessage: `The provider rejected campaign generation with HTTP ${String(response.status)}.`,
              expectedRevision: effect.revision,
              state: 'rejected',
            });
      ...
      if (representation !== undefined) {
        ...
        if (state === 'failed' || state === 'cancelled' || state === 'incomplete') {
          effect = repository.transitionEffect({
            ...
            errorMessage: `The provider ended campaign generation with status ${state}.`,
            expectedRevision: effect.revision,
            state: 'rejected',
          });
          throw rejectedEffect(effect);
...
function assertEffectIdentity(
  ...
  if (
    ...
    throw new GenerationProviderTerminalError(
      ...
      'The durable provider receipt does not match the requested campaign generation.',
      'ambiguous',
    );
  }
}
...
function rejectedEffect(effect: ProviderEffect): GenerationProviderTerminalError {
  return new GenerationProviderTerminalError(
    ...
    effect.errorMessage ?? 'The provider could not complete campaign generation.',
    'failed',
  );
}
