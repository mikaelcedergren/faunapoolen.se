// The marketing rules the campaigns tool writes by, and teaches from.
//
// These are data, not prose inside a prompt, so one rule can be referenced from three places at
// once: the model cites rule ids next to every field it writes, the generation prompt is composed
// from the same list, and the admin sidebar renders the durable lesson text below. `teaches` is
// authored here and never by the model — the point is that the user meets the same fourteen
// lessons in the same words every time, instead of fourteen fresh improvisations per campaign.
...
export const MARKETING_RULE_IDS = Object.freeze(MARKETING_RULES.map((rule) => rule.id));

const RULES_BY_ID = new Map(MARKETING_RULES.map((rule) => [rule.id, rule]));

export function marketingRule(id) {
  return RULES_BY_ID.get(id);
}

export function isMarketingRuleId(value) {
  return typeof value === 'string' && RULES_BY_ID.has(value);
}

// The same list, rendered for the model. Kept in one place so the rules the studio writes by and
// the rules it teaches can never drift apart.
export function marketingRulesPromptBlock() {
